...
import { BW, CapturableSymbol, GameTerminationEvent, Player, PromotionSymbol } from "@common/src/types/game"
...
type RdcrActnEnd = ReducerAction<"END",GameTerminationEvent>
...
function reducer<A extends GameRdcrActn>(state:GAMECONTEXT,action:A) : GAMECONTEXT {
    ...
    switch(type)
    {
        ...
        case "MOVE":
            ...
            {
                state.game.engine.clock.editDuration("w",payload.time.remaining.w)
                state.game.engine.clock.editDuration("b",payload.time.remaining.b)
                state.game.engine.clock.switch(true,state.game.engine.instance.turn())
            }
