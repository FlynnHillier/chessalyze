...
import { GameIsPresentException, LobbyIsPresentException,GameIsNotPresentException, LobbyIsNotPresentException } from "../../../types/errors"
...
export const inLobby = (req:Request,res:Response,next:NextFunction) => {
    const exisitingLobby = GameManager.getPlayerLobby(req.user!.uuid)
    if(!exisitingLobby){
        return next(new LobbyIsNotPresentException())
    }
    next()
}
