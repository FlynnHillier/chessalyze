...
import { createGameRoom, destroyGameRoom } from "~/lib/ws/rooms/game.room.ws"
...
class GameInstanceManagerClass {
  ...
  public getPlayerGame(uuid: UUID): null | GameInstance {
    const game = this.gameInstances.find((game) => game.players.w.pid === uuid || game.players.b.pid === uuid)
  ...
  public createLobby(player: Player): Lobby {
    const newLobby: Lobby = { //create new lobby
      player,
      id: uuidv1()
    }
